...
// C. Tomlinson 02/14/2020 AT24C02C 400kMHz max
...
//using existing 1024 400kHz timing set for AT24C02C C.Tomlinson 01/28/2020
...
// AT24C32E, 128 pages at 32 bytes/page, 12 address bits
`ifdef AT24C32E
  parameter MEM_SIZE = 4096;	 	
  parameter ADDR_SIZE = 12;
  parameter WORD_ADDR_SIZE = 5;
...
// AT24C02C, 32 pages at 8 bytes/page, 8 address bits
`ifdef AT24C02C
...
`ifdef AT24C02
  ...
  parameter ADDR_SIZE = 8;
  parameter WORD_ADDR_SIZE = 3;
  parameter SLAVE_ADDRESS = 7'b1010_000; // Testbench provides R/W bit
`endif
...
// AT24C64, AT24C64A 256 pages at 32 bytes, 13 address bits 
parameter MEM_SIZE_64K_BIT = 8192;		 
parameter ADDR_SIZE_64K_BIT = 13;
parameter WORD_ADDR_SIZE_64K_BIT = 5;

// AT24C32, AT24C32A, AT24C32E 128 pages at 32 bytes/page, 12 address bits 
...
// AT24C02, AT24C02A, AT24C0C 32 pages at 8 bytes/page, 8 address bits 
...
always @ (negedge SCL) begin: Watch_for_Random_START_trigger
  if(S_START && First_START_flag)begin
    ...
    if(VERBOSE) $display(">>> Detected random START n protocol interupt", $time);
...
`ifdef AT24C02C
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C02C");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
    $display ("Valid device address not detected for AT24C02C");
  end
`endif
...
`ifdef WANT_AT24C1024__4_5__5_5V_TIMING
  ...
  $hold(negedge SDA, negedge SCL, tHDSTA);		// START hold time  cjt 02_26_2020
  $setup(posedge SCL, negedge SDA &&& en_st_stp_timing, tSUSTA);	// START setup time  cjt 02_26_2020


  $hold(negedge SCL, SDA, tHDDAT);		// Data_in hold time  
  $setup(SDA, posedge SCL, tSUDAT);		// Data_in setup time  

  $setup(posedge SCL, posedge SDA &&& en_st_stp_timing, tSUSTO);	// STOP setup time cjt 02_26_2020  
endspecify
`endif
